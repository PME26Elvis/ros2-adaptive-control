...
namespace adaptive_controllers::param_utils {
...
inline bool read_matrix(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
  ...
  if (!node->has_parameter(key)) return false;
  ...
  if (data.size() != rows * cols) {
    RCLCPP_ERROR(node->get_logger(), "[%s] size %zu != %zu*%zu",
...
inline bool read_vector(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
  ...
  const auto data = param.as_double_array();
  if (data.size() != len) {
    RCLCPP_ERROR(node->get_logger(), "[%s] size %zu != %zu",
